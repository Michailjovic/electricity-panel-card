...
function ntRemainingMinsFromWindows(windows, dayEnd, now) {
  let rem = 0;
  for (const w2 of windows) {
    const en = Math.min(w2.end, dayEnd);
    if (en > w2.start && now < en) rem += (en - Math.max(now, w2.start)) / 6e4;
  }
  return rem;
}
function ntWindowsForDay(day, base, dayEnd) {
  ...
  }).filter((w2) => w2.end > w2.start && w2.start < dayEnd).sort((a2, b2) => a2.start - b2.start);
}
function buildFullDaySlotsFromWindows(ntWindows, base, dayEnd, showing, now, fmt) {
...
function resolveHdoStatus(now, switchOn, switchSince, windows, dayEnd) {
  const current = windows.find((w2) => now >= w2.start && now < w2.end);
  const prev = [...windows].reverse().find((w2) => w2.end <= now);
  const next = windows.find((w2) => w2.start > now);
...
function isNTAt(t2, hdoHistory, windows, fallbackSwitchOn) {
  ...
  if (windows) {
    return windows.some((w2) => t2 >= w2.start && t2 < w2.end);
  }
  return fallbackSwitchOn;
}
function parseScheduleEntity(attributes, dayStart, dayEnd) {
  const raw = attributes == null ? void 0 : attributes["schedule"];
  if (!Array.isArray(raw)) return void 0;
  const pick = (o2, keys) => {
    for (const k2 of keys) if (o2[k2] !== void 0) return o2[k2];
    return void 0;
  };
  const isNtEntry = (o2) => {
    const tariff = pick(o2, ["tariff", "type", "rate"]);
    if (typeof tariff === "string") {
      const t2 = tariff.trim().toUpperCase();
      if (t2 === "NT" || t2.includes("LOW")) return true;
      if (t2 === "VT" || t2.includes("HIGH")) return false;
    }
    const value = pick(o2, ["value", "is_nt", "is_low", "low"]);
  ...
  for (const entry of raw) {
    ...
    const o2 = entry;
    if (!isNtEntry(o2)) continue;
    const startRaw = pick(o2, ["start", "from", "begin"]);
    const endRaw = pick(o2, ["end", "to", "until"]);
    if (typeof startRaw !== "string" || typeof endRaw !== "string") continue;
    const s2 = new Date(startRaw).getTime();
    const e2 = new Date(endRaw).getTime();
    if (isNaN(s2) || isNaN(e2) || e2 <= s2) continue;
    const start = Math.max(s2, dayStart);
    const end = Math.min(e2, dayEnd);
    if (end > start) windows.push({ start, end });
  }
  return windows.sort((a2, b2) => a2.start - b2.start);
}
...
function ntFractionOfInterval(start, end, hdoHistory, windows, fallbackSwitchOn) {
  ...
  if (windows) {
    for (const w2 of windows) {
      if (w2.start > start && w2.start < end) breaks.add(w2.start);
      if (w2.end > start && w2.end < end) breaks.add(w2.end);
    }
  }
...
function comparePosition(a2, b2) {
  if (a2 === b2) return 0;
  if (!a2) return 1;
  if (!b2) return -1;
  const ma = /^(\d+)(.*)$/.exec(a2.trim());
  const mb = /^(\d+)(.*)$/.exec(b2.trim());
  if (ma && mb) {
    const d2 = parseInt(ma[1], 10) - parseInt(mb[1], 10);
    return d2 !== 0 ? d2 : ma[2].localeCompare(mb[2]);
  }
  if (ma) return -1;
  if (mb) return 1;
  return a2.localeCompare(b2);
}
function buildRails(mods, railSize) {
  const size = Math.max(1, Math.floor(railSize));
  const rails = [];
  let row = [];
  let used = 0;
  for (const m2 of mods) {
    const w2 = Math.max(1, Math.floor(m2.width));
    if (row.length && used + w2 > size) {
      rails.push(row);
      row = [];
      used = 0;
    }
...
function loadPercent(amps, watts, maxAmps, volts = 230) {
...
let ElectricityPanelEditor = class extends i {
  ...
  _renderLayoutSection() {
    ...
    const p2 = this._config.panel ?? {};
    const sp = (f2) => (v2) => {
      const n3 = parseFloat(v2);
      this._set(["panel", f2], v2 === "" ? "" : isNaN(n3) ? v2 : n3);
    };
    return b`
      <details class="section">
        ...
        <div class="section-body">
          <div class="field">
            <label>View</label>
            <select @change=${(e2) => this._set(["view"], e2.target.value)}>
          ...
          ${isPanel ? b`
            ${this._numField("Positions per rail", p2.rail_size, sp("rail_size"), "12")}
            ${this._numField("Main breaker rating (A)", p2.main_breaker, sp("main_breaker"), "25")}
            <div class="field checkbox">
              <input type="checkbox" id="pnl-spark" .checked=${p2.module_spark !== false}
                @change=${(e2) => this._set(["panel", "module_spark"], e2.target.checked)} />
              <label for="pnl-spark">Micro graph inside each module</label>
            </div>
            <div class="field checkbox">
              <input type="checkbox" id="pnl-main" .checked=${p2.show_main !== false}
                @change=${(e2) => this._set(["panel", "show_main"], e2.target.checked)} />
  ...
  _activeScheduleSourceLabel() {
    var _a2;
    const h2 = this._config.hdo;
    if (!h2) return "none configured";
    const fallback = h2.tariff_preset ? `tariff_preset (${h2.tariff_preset})` : h2.schedule ? "manual schedule" : "none configured";
    if (h2.schedule_entity) {
      const entity = (_a2 = this.hass) == null ? void 0 : _a2.states[h2.schedule_entity];
      if (entity) {
        const midnight = /* @__PURE__ */ new Date();
        midnight.setHours(0, 0, 0, 0);
        const start = midnight.getTime();
        const windows = parseScheduleEntity(entity.attributes, start, dayEndMs(start));
        if (windows) return `schedule_entity (${h2.schedule_entity})`;
      }
    ...
    return b`
      <details class="section">
        ...
        <div class="section-body">
          ...
          ${this._entityField("Schedule entity (NT windows — advanced)", h2.schedule_entity, s2("schedule_entity"))}
          <span class="field-hint">
      ...
      this.requestUpdate();
        ...
        ${open ? b`
          <div class="sub-fields">
            ...
            ${this._textField("Position in board (panel view)", c2.position, sf("position"), "e.g. 08")}
            ${c2.phases === 3 ? A : b`
              <div class="field">
                <label>Phase (panel view)</label>
                <select @change=${(e2) => this._setCircuitField(idx, "phase", e2.target.value)}>
                  <option value="" ?selected=${!c2.phase}>— not set —</option>
                  <option value="L1" ?selected=${c2.phase === "L1"}>L1</option>
                  <option value="L2" ?selected=${c2.phase === "L2"}>L2</option>
...
let ElectricityPanelCard = class extends i {
  ...
  _fmtW(w2) {
    if (w2 >= 1e3) return `${(w2 / 1e3).toFixed(2)} kW`;
    return `${w2.toFixed(0)} W`;
  }
  ...
  _fmtPrice(price) {
    const n3 = parseFloat(price);
    return isNaN(n3) ? "" : n3.toFixed(2);
  }
  _kwh(entityId) {
    var _a2;
    if (!entityId) return 0;
  ...
  _ageBadge(entityId) {
    ...
    const color = diffMs >= staleMs ? this._config.age_stale_color ?? "#ef4444" : diffMs >= warnMs ? this._config.age_warn_color ?? "#f59e0b" : this._config.age_ok_color ?? "var(--ep-text-faint)";
    return b`<span class="age-badge" style="color:${color}">↻ ${label}</span>`;
  }
  ...
  _scheduleWindows(dayOffset) {
    var _a2;
    const hdo = this._config.hdo;
    if (!hdo) return void 0;
    const { start, end } = this._dayBounds(dayOffset);
    if (hdo.schedule_entity) {
      const entity = (_a2 = this.hass) == null ? void 0 : _a2.states[hdo.schedule_entity];
      const windows = entity ? parseScheduleEntity(entity.attributes, start, end) : void 0;
      if (windows) {
  ...
  _hdoWindowsToday() {
    ...
    if (!hdo || !today) return void 0;
    let windows = today.windows;
    if (hdo.merge_midnight && windows.length) {
      const last = windows[windows.length - 1];
      if (last.end === today.end) {
        const firstTomorrow = (_a2 = this._scheduleWindows(1)) == null ? void 0 : _a2.windows[0];
  ...
  _renderSparkWindowSwitch(inline = false) {
  ...
  _scheduleData() {
    ...
    let totalNT = windows.reduce((a2, w2) => a2 + (w2.end - w2.start) / 6e4, 0);
    if (!showing && hdo.merge_midnight && windows.length) {
      const last = windows[windows.length - 1];
      if (last.end === dayEnd) {
        const firstTomorrow = (_a2 = this._scheduleWindows(1)) == null ? void 0 : _a2.windows[0];
        if (firstTomorrow && firstTomorrow.start === dayEnd) {
          const extra = (firstTomorrow.end - firstTomorrow.start) / 6e4;
  ...
  _renderDayTimeline() {
    ...
    if (!data) return A;
    const left = data.remaining !== null ? b`<span class="nt-remaining">${this._fmtMins(data.remaining)} ${this._t("nt_left")}</span>` : A;
    return b`
      <div class="day-timeline">
        ${this._renderTimeline(data.slots, !data.showing)}
        ${left !== A ? b`<div class="day-timeline-foot">${left}</div>` : A}
      </div>`;
  }
  _renderHdoSchedule() {
    const data = this._scheduleData();
    ...
    return b`
      ...
      this._showTomorrow = !this._showTomorrow;
          ...
          ${this._isPanelView() ? A : this._renderTimeline(slots, !showing)}
          ${exp ? b`
        ...
        <div class="meter-header">
          ...
          <div class="meter-total">
            ...
            <span class="metric-small">
              ${this._metricRow([
      m2.energy_today ? b`${this._kwh(m2.energy_today).toFixed(1)} ${this._t("kwh_today")}` : null,
      (() => {
        const cr = this._calcDailyCost(m2.energy_today, m2.power_l1, m2.power_l2, m2.power_l3);
        return cr ? b`<span class="cost-rate">${cr}</span>` : null;
      })(),
      m2.voltage && voltage > 0 ? b`${voltage.toFixed(0)} V` : null,
      this._ageBadgePart(m2.power_l1 ?? m2.power_l2 ?? m2.power_l3 ?? m2.energy_today)
    ])}
            </span>
          </div>
        </div>
      ...
      <div class="circuit-card ${c2.critical ? "critical" : ""} ${c2.switch && isOn ? "is-on" : ""}">
        ...
        <div class="circuit-footer">
          <div class="metrics">
            ...
            <span class="metric-small">
              ${this._metricRow([
      c2.current ? b`${this._isAvail(c2.current) ? current.toFixed(1) : "—"} A` : null,
      c2.voltage ? b`${this._num(c2.voltage).toFixed(0)} V` : null,
      energy > 0 ? b`${energy.toFixed(2)} kWh` : null,
      costRate ? b`<span class="cost-rate">${costRate}</span>` : null,
      this._ageBadgePart(c2.power ?? c2.current ?? c2.switch)
    ])}
            </span>
          </div>
  ...
  _renderThreePhaseCircuit(c2) {
    ...
    return b`
      ...
      energy > 0 ? b`${energy.toFixed(2)} kWh` : null,
      costRate ? b`<span class="cost-rate">${costRate}</span>` : null,
      this._ageBadgePart(c2.power ?? c2.power_l1 ?? c2.current_l1 ?? c2.switch)
    ])}
            </span>
          </div>
        </div>
  ...
  _isPanelView() {
    var _a2;
    return ((_a2 = this._config) == null ? void 0 : _a2.view) === "panel";
  }
  ...
  _panelCfg() {
    const p2 = this._config.panel ?? {};
    return {
      railSize: p2.rail_size ?? 12,
      mainBreaker: p2.main_breaker ?? 25,
      moduleSpark: p2.module_spark !== false,
      showMain: p2.show_main !== false
    };
  }
  /**
  ...
  _panelModules() {
    ...
    const mm = this._config.main_meter;
    if (cfg.showMain && mm) {
      const w2 = this._watts(mm.power_l1) + this._watts(mm.power_l2) + this._watts(mm.power_l3);
      const maxPhaseA = Math.max(
        this._num(mm.current_l1),
        this._num(mm.current_l2),
        this._num(mm.current_l3)
      );
      ...
      mods.push({
        ...
        pct: loadPercent(maxPhaseA, w2 / 3, cfg.mainBreaker, this._mainVoltage()),
        isOn: true,
        hasSwitch: false,
        critical: true,
    ...
    const circuits = (this._config.circuits ?? []).slice().sort((a2, b2) => comparePosition(a2.position, b2.position));
    for (const c2 of circuits) {
      const three = c2.phases === 3;
      const watts = three ? c2.power ? this._watts(c2.power) : this._watts(c2.power_l1) + this._watts(c2.power_l2) + this._watts(c2.power_l3) : this._watts(c2.power);
      const amps = three ? c2.current ? this._num(c2.current) : Math.max(this._num(c2.current_l1), this._num(c2.current_l2), this._num(c2.current_l3)) : this._num(c2.current);
      const maxA = c2.max_current ?? (three ? 63 : 16);
      const volts = this._num(c2.voltage) || this._num(c2.voltage_l1) || 230;
      mods.push({
        id: c2.id,
        width: three ? 3 : 1,
        position: c2.position ?? "",
        name: c2.name,
        phaseCls: three ? "l3f" : c2.phase ? c2.phase.toLowerCase() : "none",
        watts,
        amps,
        maxA,
        ...
        ...three && !c2.power ? (() => {
          return {
            sparkId: this._dominantPhase(
              ...
              [c2.current_l1, c2.current_l2, c2.current_l3]
            ).id
          };
        })() : { sparkId: c2.power },
        circuit: c2
   ...
   * total.
   */
  _dominantPhase(powers, currents) {
    let best = -1, bestVal = -1;
    for (let i2 = 0; i2 < 3; i2++) {
      if (!powers[i2]) continue;
      const v2 = this._num(currents[i2]) || this._watts(powers[i2]);
      if (v2 > bestVal) {
        bestVal = v2;
        best = i2;
      }
   ...
   */
  _metricRow(parts) {
    const kept = parts.filter((x2) => x2 !== null && x2 !== void 0 && x2 !== "");
    return b`${kept.map((part, i2) => b`${i2 > 0 ? b`<span class="metric-sep">·</span>` : A}${part}`)}`;
  }
  ...
  _ageBadgePart(entityId) {
    const b2 = this._ageBadge(entityId);
    return b2 === A ? null : b2;
  }
  _mainVoltage() {
    const mm = this._config.main_meter;
  ...
  // ── panel detail ───────────────────────────────────────────────────────────
  _renderPanelTools() {
    const n3 = this._panelPick.length;
    return b`
      <div class="g-bar">
        ...
        ${n3 > 1 ? b`
          <button class="g-chk ${this._panelSharedY ? "on" : ""}" role="switch"
            ...
            @click=${() => {
      this._panelSharedY = !this._panelSharedY;
    }}>
            <span class="box"></span>${this._t("shared_axis")}
          </button>` : A}
        <span class="g-spacer"></span>
        ${n3 ? b`
          <button class="g-btn" @click=${() => {
      this._panelPick = [];
    }}>
            ${this._t("clear_selection", { n: String(n3) })}
          </button>` : A}
      </div>`;
  }
  _renderPanelDetail(mods) {
    const picked = this._panelPick.map((id) => mods.find((m2) => m2.id === id)).filter(Boolean);
    let body;
    if (picked.length === 0) {
      body = b`<div class="rd-hint">
    ...
    return b`<div class="rail-detail">${this._renderPanelTools()}${body}</div>`;
  }
  _renderPanelSingle(m2) {
    const c2 = m2.circuit;
    const three = m2.width === 3;
    const energy = c2 ? this._kwh(c2.energy) : this._config.main_meter ? this._kwh(this._config.main_meter.energy_today) : 0;
    const costRate = c2 ? c2.phases === 3 && !c2.power ? this._calcDailyCost(c2.energy, c2.power_l1, c2.power_l2, c2.power_l3) : this._calcDailyCost(c2.energy, c2.power) : this._config.main_meter ? this._calcDailyCost(
      this._config.main_meter.energy_today,
      this._config.main_meter.power_l1,
      this._config.main_meter.power_l2,
      this._config.main_meter.power_l3
    ) : "";
    return b`
      <div class="rd-head">
        ${m2.position ? b`<span class="rd-num">${m2.position}</span>` : A}
        <span class="rd-name">${m2.name}</span>
        ${three ? b`<span class="badge badge-phase">3φ</span>` : A}
        ${m2.critical ? b`<ha-icon icon="mdi:lock" class="lock-icon"></ha-icon>` : m2.hasSwitch && c2 ? b`<button class="toggle ${m2.isOn ? "on" : "off"}"
                @click=${() => this._toggle(c2.switch, c2.name, c2.confirm_toggle)}
      ...
      ${three ? this._renderPanelPhases(m2) : b`<div class="rd-graph">${this._renderSparkline(m2.sparkId)}</div>`}
      ${this._renderPanelDevices(m2)}`;
  }
  /**
   ...
   */
  _renderPanelPhases(m2) {
    const c2 = m2.circuit;
    const mm = this._config.main_meter;
    const phases = c2 ? [
      { lbl: "L1", power: c2.power_l1, current: c2.current_l1, voltage: c2.voltage_l1 },
      { lbl: "L2", power: c2.power_l2, current: c2.current_l2, voltage: c2.voltage_l2 },
      { lbl: "L3", power: c2.power_l3, current: c2.current_l3, voltage: c2.voltage_l3 }
    ] : [
      { lbl: "L1", power: mm == null ? void 0 : mm.power_l1, current: mm == null ? void 0 : mm.current_l1, voltage: (mm == null ? void 0 : mm.voltage_l1) ?? (mm == null ? void 0 : mm.voltage) },
      { lbl: "L2", power: mm == null ? void 0 : mm.power_l2, current: mm == null ? void 0 : mm.current_l2, voltage: (mm == null ? void 0 : mm.voltage_l2) ?? (mm == null ? void 0 : mm.voltage) },
      { lbl: "L3", power: mm == null ? void 0 : mm.power_l3, current: mm == null ? void 0 : mm.current_l3, voltage: (mm == null ? void 0 : mm.voltage_l3) ?? (mm == null ? void 0 : mm.voltage) }
    ];
    const globalMax = Math.max(
      ...phases.map((p2) => {
        var _a2;
        return p2.power ? ((_a2 = this._sparkPaths(p2.power)) == null ? void 0 : _a2.vMax) ?? 0 : 0;
      }),
      0
    );
    const globalMin = Math.min(
      ...phases.map((p2) => {
        var _a2;
        return p2.power ? ((_a2 = this._sparkPaths(p2.power)) == null ? void 0 : _a2.vMin) ?? 0 : 0;
      }),
      0
    );
    return b`
      <div class="ph3-grid">
        ${phases.map((p2) => {
      const w2 = this._watts(p2.power);
      const a2 = this._num(p2.current);
      const v2 = this._num(p2.voltage);
      const pct = loadPercent(a2, w2, m2.maxA, v2 || 230);
      const cls = p2.lbl.toLowerCase();
      return b`
  ...
  _renderScaledSpark(entityId, gMin, gMax, color) {
    if (!entityId) return A;
    const s2 = this._sparkPaths(entityId);
    if (!s2) return A;
    const H2 = 38, pad = 3, inner = H2 - pad * 2;
    const gRange = gMax - gMin;
    const lRange = s2.vMax - s2.vMin;
    let g2;
    if (!(gRange > 0) || !(lRange > 0)) {
      const y3 = gRange > 0 ? H2 - pad - (s2.vMin - gMin) / gRange * inner : H2 - pad;
      g2 = w`<path d="M 0,${y3.toFixed(1)} L 100,${y3.toFixed(1)}" fill="none"
        style="stroke:${color}" stroke-width="1.4" vector-effect="non-scaling-stroke"/>`;
    } else {
      const k2 = lRange / gRange;
      const ty = (H2 - pad) * (1 - k2) - inner / gRange * (s2.vMin - gMin);
      g2 = w`
        <g transform="translate(0 ${ty.toFixed(2)}) scale(1 ${k2.toFixed(4)})">
          <path d="${s2.line}" fill="none" style="stroke:${color}" stroke-width="1.4"
            stroke-linejoin="round" vector-effect="non-scaling-stroke"/>
        </g>`;
    }
    return b`
      <svg class="ph3-spark" viewBox="0 0 100 ${H2}" preserveAspectRatio="none">${g2}</svg>`;
  }
  /**
  ...
  _moduleSeries(m2) {
    const c2 = m2.circuit;
    const mm = this._config.main_meter;
    const per = c2 ? [c2.power_l1, c2.power_l2, c2.power_l3] : [mm == null ? void 0 : mm.power_l1, mm == null ? void 0 : mm.power_l2, mm == null ? void 0 : mm.power_l3];
    if (per.some(Boolean)) {
      return per.map((id, i2) => ({
        key: `${m2.id}:L${i2 + 1}`,
        label: `L${i2 + 1}`,
        entityId: id,
    ...
    return [{ key: m2.id, label: "", entityId: m2.sparkId, watts: m2.watts, color: "var(--ep-neutral)" }];
  }
  _renderPanelCompare(picked) {
    const groups = picked.map((m2) => ({ m: m2, series: this._moduleSeries(m2) }));
    const flat = groups.flatMap((g2) => g2.series.map((sv) => ({ ...sv, m: g2.m })));
    const paths = flat.map((e2) => e2.entityId ? this._sparkPaths(e2.entityId) : void 0);
    const gMax = Math.max(...paths.map((p2) => (p2 == null ? void 0 : p2.vMax) ?? 0), 0);
    const gMin = Math.min(...paths.map((p2) => (p2 == null ? void 0 : p2.vMin) ?? 0), 0);
    const shared = this._panelSharedY;
    const tri = groups.some((g2) => g2.series.length === 3);
    return b`
      <div class="cmp-grid ${tri ? "tri" : ""}">
        ${flat.map((e2, i2) => {
      var _a2;
      const localMax = ((_a2 = paths[i2]) == null ? void 0 : _a2.vMax) ?? 0;
      return b`
            <div class="cmp-cell">
              <div class="cmp-head">
                ${e2.m.position ? b`<span class="cmp-num">${e2.m.position}</span>` : A}
                <span class="cmp-name">${e2.m.name}</span>
                ${e2.label ? b`<span class="cmp-ph ${e2.label.toLowerCase()}">${e2.label}</span>` : A}
                <span class="cmp-val">${this._fmtW(e2.watts)}</span>
              </div>
              <div class="cmp-scale">
                max ${this._fmtW(shared ? gMax : localMax)}${shared ? b` · ${this._t("shared_axis")}` : A}
              </div>
              ${shared ? this._renderScaledSpark(e2.entityId, gMin, gMax, e2.color) : b`<div class="cmp-plain">${this._renderSparkline(e2.entityId, true)}</div>`}
            </div>`;
    })}
      </div>`;
  }
  _renderPanelDevices(m2) {
    var _a2;
    const devices = ((_a2 = m2.circuit) == null ? void 0 : _a2.devices) ?? [];
    if (!devices.length) {
      return b`<div class="dev-empty">${this._t("no_devices")}</div>`;
    }
    return b`
      <div class="dev-wrap">
        <div class="dev-title">${this._t("devices_behind")}</div>
        <div class="devices-list" style="margin-top:0;padding-top:0;border-top:none">
          ${devices.map((d2) => this._renderDevice(d2))}
        </div>
      </div>`;
  }
  // ── Main render ────────────────────────────────────────────────────────────
...
__decorateClass([
  r()
], ElectricityPanelCard.prototype, "_panelPick", 2);
__decorateClass([
  r()
